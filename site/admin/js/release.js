...
function save() {
    ...
    var data = {
        ...
        'PlayerForList': $('#PlayerForList').val(),
        'PlayerForPage': $('#PlayerForPage').val(),
    ...
    $.post('release.php', data, function(result) {
        if (IsNumeric(result)) {
            $('#LoadingSecondary').show();
            window.location = 'release.php?created=ok&id=' + result;
        } else if (result == 'OK') {
            $("#MessageText").text("Saved successfully");
...
function deleteRelease() {
    $('#Delete').click(function() {
        if (confirm("Do you really want to delete this release?")) {
            var data = {
                'action': 'delete',
                'Id': $('#Id').val()
            };
            ...
            $.post('release.php', data, function(result) {
                if (result == 'OK') {
                    $("#MessageText").text("Release has been deleted");
                    $("#Message").fadeIn('fast');
                    cancel();
                } else {
                    $("#ErrorText").html(result);
...
function cancel() {
    window.location = '/site/admin/label.php'
}
